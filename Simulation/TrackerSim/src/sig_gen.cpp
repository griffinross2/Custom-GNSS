...
GalileoSigGen::GalileoSigGen(double fs, double fc, double noise_power_db, double doppler_start, double doppler_end)
{
    this->fs = fs;
    this->fc = fc;
    ...
    this->noise = std::normal_distribution<double>(0, 1);
}

void GalileoSigGen::generate(uint8_t *signal, long long size)
{
    // NCOs
    ...
    for (long long i = 0; i < size; i++)
    {
        ...
        // MAX-2769 Wideband = 18MHz
        // Noise power = -174 + 10 * log10(18e6) ~= -101dBm
        // Minimum signal power from SIS ICD = -157.25dBW = -127.25dbm
        // Relative noise power = -101 - (-127.25) = 26.25dB maximum
        signal_sample += noise(gen) * sqrt(pow(10.0, (noise_power_db / 10.0)));

        // Hard limiting
        signal[i] = signal_sample > 0 ? 1 : 0;
    }
}