#include "sig_gen.h"
#include <math.h>
#include "stdlib.h"
#include <random>
#include "tools.h"

// Random seeding
std::random_device rd;
std::mt19937 gen(rd());

// Code for Galileo E1 pilot of PRN 1
uint8_t gal_e1_pilot_code[] = {};

// Code for Galileo E1 data of PRN 1
uint8_t gal_e1_data_code[] = {};

// Code taps for GPS L1 C/A of PRN 1
uint8_t gps_l1ca_taps[] = {2, 6};

#define PI 3.14159265358979323846

#define CHIP_RATE_E1 1.023e6
#define FREQ_E1 1.57542e9

#define CHIP_RATE_L1CA 1.023e6
#define FREQ_L1CA 1.57542e9

SignalFromFile::SignalFromFile()
{
    file = NULL;
    byte = '\0';
    nbyte = 0;
    nbit = 0;
}

bool SignalFromFile::open(const char *filename)
{
    file = fopen(filename, "rb");
    return file != NULL;
}

void SignalFromFile::close()
{
    if (file != NULL)
    {
        fclose(file);
        file = NULL;
    }
}

void SignalFromFile::generate(double *signal, long long size)
{
    for (long long i = 0; i < size; i++)
    {
        // Top of byte
        if (nbit == 0)
        {
            if (fread(&byte, sizeof(char), 1, file) != 1)
                break;
        }

        // Get signal bit
        signal[i] = ((byte >> nbit) & 0x1) ? 1.0 : -1.0;
        nbit = (nbit + 1) % 8;
        if (nbit == 0)
            nbyte++;
    }
}

NoiseGen::NoiseGen(double fs, double fc, double bandwidth)
{
    this->fs = fs;
    this->fc = fc;
    this->bandwidth = bandwidth;
    noise = std::normal_distribution<double>(0.0, 1.0);
}

void NoiseGen::generate(double *signal, long long size)
{
    // Thermal noise floor ~-174dBm / Hz
    // Noise power = -174 + 10 * log10(bandwidth) ~= -101dBm
    double noise_power_dbm = -174.0 + 10.0 * log10(bandwidth);
    for (long long i = 0; i < size; i++)
    {
        // Generate Gaussian noise
        double noise_sample = noise(gen);
        signal[i] = sqrt(pow(10, (noise_power_dbm / 10.0))) * noise_sample;
    }
}

GalileoE1SigGen::GalileoE1SigGen(double fs, double fc, double power_dbm, double doppler_start, double doppler_end)
{
    this->fs = fs;
    this->fc = fc;
    this->power_dbm = power_dbm;
    this->doppler_start = doppler_start;
    this->doppler_end = doppler_end;
}

void GalileoE1SigGen::generate(double *signal, long long size)
{
    // NCOs
    double carrier_phase = 0.0;
    double code_phase = 0.0;
    double carrier_rate = (fc + doppler_start) / fs;
    double code_rate = (CHIP_RATE_E1 + (doppler_start * CHIP_RATE_E1 / FREQ_E1)) / fs;

    // Code chip
    int code_index = 0;

    // Data bit
    double data_bit = 1.0;

    // Generate the signal
    for (long long i = 0; i < size; i++)
    {
        // Update doppler
        double curr_doppler = doppler_start + (doppler_end - doppler_start) * (double)i / size;
        carrier_rate = (fc + curr_doppler) / fs;
        code_rate = (CHIP_RATE_E1 + (curr_doppler * CHIP_RATE_E1 / FREQ_E1)) / fs;

        // Generate the carrier
        double carrier = cos(2 * PI * carrier_phase);
        carrier_phase += carrier_rate;
        if (carrier_phase >= 1)
            carrier_phase -= 1.0;

        // Generate the code
        double code_sample = ((gal_e1_data_code[int(code_index / 8)] >> (7 - (code_index % 8))) & 0x1) ? 1.0 : -1.0;
        double code_sample_pilot = ((gal_e1_pilot_code[int(code_index / 8)] >> (7 - (code_index % 8))) & 0x1) ? 1.0 : -1.0;
        code_phase += code_rate;
        if (code_phase >= 1)
        {
            if (code_index == 4091)
            {
                code_index = 0;
                data_bit = (rand() % 2) * 2.0 - 1.0;
            }
            else
            {
                code_index += 1;
                code_phase -= 1.0;
            }
        }

        // Modulate data onto the data code
        code_sample *= data_bit;

        // Modulate BOC onto both codes
        code_sample *= code_phase > 0.5 ? 1.0 : -1.0;
        code_sample_pilot *= code_phase > 0.5 ? 1.0 : -1.0;

        // Combine the signals
        double signal_sample = (1.0 / sqrt(2)) * (carrier * code_sample + carrier * code_sample_pilot);

        // Scale power
        signal_sample *= sqrt(pow(10, (power_dbm / 10.0)));

        signal[i] = signal_sample;
    }
}

GPSL1CASigGen::GPSL1CASigGen(double fs, double fc, double power_dbm, double doppler_start, double doppler_end)
{
    this->fs = fs;
    this->fc = fc;
    this->power_dbm = power_dbm;
    this->doppler_start = doppler_start;
    this->doppler_end = doppler_end;
}

void GPSL1CASigGen::generate(double *signal, long long size)
{
    // NCOs
    double carrier_phase = 0.0;
    double code_phase = 0.0;
    double carrier_rate = (fc + doppler_start) / fs;
    double code_rate = (CHIP_RATE_L1CA + (doppler_start * CHIP_RATE_L1CA / FREQ_L1CA)) / fs;

    // CA code generator
    CACodeGenerator ca_code_gen(gps_l1ca_taps[0], gps_l1ca_taps[1]);

    // Code chip
    int code_index = 0;

    // Data bit
    int data_index = 0;
    double data_bit = 1.0;

    // Generate the signal
    for (long long i = 0; i < size; i++)
    {
        // Update doppler
        double curr_doppler = doppler_start + (doppler_end - doppler_start) * (double)i / size;
        carrier_rate = (fc + curr_doppler) / fs;
        code_rate = (CHIP_RATE_L1CA + (curr_doppler * CHIP_RATE_L1CA / FREQ_L1CA)) / fs;

        // Generate the carrier
        double carrier = cos(2 * PI * carrier_phase);
        carrier_phase += carrier_rate;
        if (carrier_phase >= 1)
            carrier_phase -= 1.0;

        // Generate the code
        double code_sample = ca_code_gen.get_chip() ? 1.0 : -1.0;
        code_phase += code_rate;
        if (code_phase >= 1)
        {
            ca_code_gen.clock_chip();
            if (code_index == 1022)
            {
                data_index += 1;
                code_index = 0;
                if (data_index == 20)
                {
                    data_index = 0;
                    data_bit = (rand() % 2) * 2.0 - 1.0;
                }
            }
            else
            {
                code_index += 1;
                code_phase -= 1.0;
            }
        }

        // Modulate data onto the data code
        code_sample *= data_bit;

        // Create the signal
        double signal_sample = carrier * code_sample;

        // Scale power
        signal_sample *= sqrt(pow(10, (power_dbm / 10.0)));

        signal[i] = signal_sample;
    }
}